...
SERVICE_CONTROLS_PORT="${SERVICE_CONTROLS_PORT:-4175}"
SERVICE_HTTP_DIRECTORY="${SERVICE_HTTP_DIRECTORY:-}"
PARSEBOX_CONFIG_FILE="${PARSEBOX_CONFIG_FILE:-}"
CURRENT_APP_ID="${CURRENT_APP_ID:-parsebox-kiosk}"
...
CURRENT_APP_ID_JSON="$(escape_json_string "${CURRENT_APP_ID}")"
...
cat >"${PARSEBOX_CONFIG_FILE}" <<EOF
{
  ...
  "controlsPort": ${SERVICE_CONTROLS_PORT},
  "staticDir": "${SERVICE_HTTP_DIRECTORY_JSON}",
  "appUrl": "${APP_URL_JSON}",
  "currentAppId": "${CURRENT_APP_ID_JSON}",
...
REMOTE_RUNNER="APP_URL=${APP_URL@Q} WAIT_URL=${WAIT_URL@Q} FORCE=${FORCE@Q} REPO_URL=${REPO_URL@Q} REPO_DIR=${REPO_DIR@Q} SERVICE_HTTP_PORT=${SERVICE_HTTP_PORT@Q} SERVICE_CONTROLS_PORT=${SERVICE_CONTROLS_PORT@Q} SERVICE_HTTP_DIRECTORY=${SERVICE_HTTP_DIRECTORY@Q} PARSEBOX_CONFIG_FILE=${PARSEBOX_CONFIG_FILE@Q} CURRENT_APP_ID=${CURRENT_APP_ID@Q} XINITRC_TEMPLATE_B64=${XINITRC_TEMPLATE_B64@Q} PROFILE_HOOK_TEMPLATE_B64=${PROFILE_HOOK_TEMPLATE_B64@Q} SERVICE_TEMPLATE_B64=${SERVICE_TEMPLATE_B64@Q} bash -c \"printf %s ${REMOTE_SCRIPT_B64@Q} | base64 -d | bash\""
